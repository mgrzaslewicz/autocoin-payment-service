...
class GetSubscriptionHandler(
    ...
    private fun BigDecimal.asScaledString() = setScale(8, RoundingMode.HALF_EVEN).toPlainString()
    ...
    override val httpHandler = HttpHandler {
        ...
        if (subscriptionCode.isNullOrBlank()) {
            ...
            if (subscription != null) {
                val btcAmount = if (subscription.btcReceiverAddress != null) priceService.getCurrencyAmountGivenUsdAmount("BTC", subscription.usdAmount.toBigDecimal()).asScaledString() else null
                val ethAmount = if (subscription.ethReceiverAddress != null) priceService.getCurrencyAmountGivenUsdAmount("ETH", subscription.usdAmount.toBigDecimal()).asScaledString() else null
