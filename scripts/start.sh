...
preconditions() {
    ...
    if [[ -z ${VERSION} ]]; then
        ...
        VERSION="autocoin-payment-service-${VERSION}";
    fi
    ...
    if [[ -z ${APP_TICKERS_PATH} ]]; then
        echo "APP_TICKERS_PATH not set. Please edit env.properties file in deployment directory.";
        exit 106;
    fi
...
docker run --name ${SERVICE_NAME} -d \
    ...
    -v ${LOG_PATH}:/app/log \
    -v ${APP_TICKERS_PATH}:/app/data \
    --memory=200m \
    --restart=no \
    --network autocoin-services-admin \
    localhost:5000/file-arbitrage-monitor:${VERSION}
